...
class BodyPartsMeasurementDataGenerator(tf.keras.utils.Sequence) :
    ...
    def data_generation(self, indexes):
        ...
        # ✅ 수정 !!!!!!!!!!!! 아래 1 -> 14로   & self.input_shape[1]//2 을 그냥 self.input_shape[1]으로   
        if self.is_with_seg :
            batch_segs = np.empty((self.batch_size, self.input_shape[0], self.input_shape[1], 1), dtype=np.float32)
            # batch_segs = np.empty((self.batch_size, self.input_shape[0]//2, self.input_shape[1]//2, 1), dtype=np.float32)
            # batch_segs = np.empty((self.batch_size, self.input_shape[0]//2, self.input_shape[1]//2, self.num_seg_channels), dtype=np.float32)        
        # Generate data
    ...
    def get_segmentation_map(self, filename, img_shape, input_shape) :
        ...
        # segs = tf.reshape(segs, (-1, 1))          # 수정
        # segs= segs[segs != 0]                     # 추가한 것 
        
        # 해당되는 pixel -> seg_classes로 채워짐
        ...
        tmp = segs.flatten()
        seg_classes = np.unique(tmp[tmp != 0])
        
        # # 몸통
        # body_segs = np.where(segs != 3, 0, segs)        # 몸통이 아닌 것은 0으로 놔두고, 몸통은 4 그대로 놔둠
        # body_arr = body_segs.flatten()
        # body_unique = np.unique(body_arr[body_arr != 0])
        
        # body_segs = (body_segs / 3)               # body_segs는 0 ~ 255 값을 가지게 됨 -> 원본 image와 같은 형태로  
        # body_segs = cv2.resize(body_segs, (input_shape[0],input_shape[1]))            # 256 x 256으로 수정
        
        # body_segs = np.expand_dims(body_segs, axis=2)
        
        
        def process_segment(segs, segment_value, input_shape):
            segs_processed = np.where(segs != segment_value, 0, segs)
            segs_processed = (segs_processed / segment_value)
            segs_processed = cv2.resize(segs_processed, (input_shape[0], input_shape[1]))
            segs_processed = np.expand_dims(segs_processed, axis=2)
            return segs_processed

        # 몸통
        body_segs = process_segment(segs, 3, input_shape)

        # 위 왼쪽다리
        L_Thigh_segs = process_segment(segs, 13, input_shape)

        # 위 오른쪽다리
        R_Thigh_segs = process_segment(segs, 14, input_shape)

        # segs = cv2.resize(segs, (input_shape[0]//2,input_shape[1]//2))        # 원코드 -> 원래 128 x 128
        # segs = np.expand_dims(segs, axis=2)                               # 원코드
    ...
    def get_list_data(self) :
        ...
        #     list_user_dirs = list_user_dirs[:8]  # Train은 처음 4개의 폴더
        # elif self.data_type == "validation":
        #     list_user_dirs = list_user_dirs[:1]  # Validation은 처음 1개의 폴더
        # elif self.data_type == "test":
        #     list_user_dirs = list_user_dirs[:1]  # Test는 처음 1개의 폴더
        ...
        for user_dir in list_user_dirs :
            try :
                ...
                # Camera info ✅ 수정 : 앞 4 / 옆 12, 28
                list_path_images = [img for img in list_path_images if img[-6:-4] == '04' or img[-6:-4] == '12' or img[-6:-4] == '28']
